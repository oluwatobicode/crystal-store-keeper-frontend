import { Loader2, TriangleAlert, PackageOpen } from "lucide-react";
...
const DashboardLowStock = () => {
  ...
  const { lowStocks } = useDashboard();

  const isLoading = lowStocks?.isLoading;
  const littleStocks = lowStocks?.data?.data;
  const { data: lowStocksData } = littleStocks || {};
    ...
    <div className="w-full bg-white border border-[#E2E4E9] h-auto px-5 py-10 rounded-[9px]">
      <div className="flex flex-col ">
        ...
        <div className="flex flex-col gap-6">
          {isLoading ? (
          ...
          ) : !lowStocksData || lowStocksData.length === 0 ? (
            ...
            lowStocksData.map((el: LowStock, index: number) => (
              <div
              ...
              >
                ...
                <button
                  ...
                  className="cursor-pointer px-3 py-1.5 border-2 border-[#EEEFF2] transition-colors rounded-[8px] text-[12px] font-medium text-[#000000]"
                >
                  Reorder
                </button>
              </div>
